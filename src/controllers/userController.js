...
const handleEditProfile = async(req, res) => {
    ...
    const userData = await userService.handleEditProfile(req.session.idUser, username, fullname, bio, birth, gender);
    return res.status(200).json({
        errCode: userData.errCode,
        message: userData.errMessage,
        user: userData.user ? userData.user : {},
...
const handleChangePassword = async(req, res) => {
    ...
    if(req.session.user_session.username) {
        const userData = await userService.handleChangePassword(req.session.user_session.username, currentpassword, newpassword);
        return res.status(200).json({
            errCode: userData.errCode,
            message: userData.errMessage
        });
...
const handleCheckFriendRelation = async (req, res) => {
    ...
    const relation = await userService.handleCheckFriendRelation(req.query.idAcc1, req.query.idAcc2);
    return res.status(200).json({
