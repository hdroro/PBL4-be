...
async function handleGetRandomUserMatching(idUser, onlineUsers) {
    try {
        ...
        let Listcompatibility = await compatibilityModel.getRateCompatibility(idZodiac); // lay ra danh sach idZodiac tuong thich
        Listcompatibility = Array.from(Listcompatibility);
        console.log('zodiac', Listcompatibility);

        let ListUserDataOnline = [];
        for(let i = 0; i < onlineUsers.length; i++) {
            const conversationModel = new Conversation("", idUser, onlineUsers[i].userID);
            const conversationCheck = await conversationModel.checkExistedConversation();
            if (!conversationCheck) {
                const userMatchData = await accountModel.getCusByID(onlineUsers[i].userID);
                if(userMatchData) {
                    ListUserDataOnline = [...ListUserDataOnline, {
        ...
        if(ListUserDataOnline.length === 0) {
            ...
            randomData.errMessage = 'Hien khong co user nao online thich hop de matching!';
            return randomData;
        }
