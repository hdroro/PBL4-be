    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(idZodiac) {
                ...
                let addUserCheck = await user.editAccount(id, username, name, bio, birthday, gender, idZodiac);
                if(addUserCheck) {
                    userDataInfo.errCode = 0;
...
let handleAddFriendRelation = (idAcc1, idAcc2) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            const checkFriendRelation = await relation.checkFriendRelation(idAcc1, idAcc2);
            if(!checkFriendRelation) {
                ...
                relationInfo.errMessage = 'Friend existed! Can not add new friend relation!';
            }
...
let handleGetListUser = async(page)=>{
    try{
        ...
        if(pageNumber<1 || pageNumber>lastPage) throw({
            errCode: 3,
